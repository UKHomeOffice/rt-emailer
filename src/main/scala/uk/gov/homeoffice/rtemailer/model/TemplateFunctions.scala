...
case class TList(list: List[String]) extends TemplateLookup {
  override def stringValue() = Left(GovNotifyError("Use csvList function to stringify TList object"))
}
case class TDate(date: DateTime) extends TemplateLookup {
  override def stringValue() = Right(date.toString("dd MMMM yyyy"))
}
...
class TemplateFunctions(implicit appContext: AppContext) {
  ...
  def applyStringFunction(value: TString, function: String): Either[GovNotifyError, TemplateLookup] = {
    ...
    Try(function match {
      ...
      case "parseDate" =>
        ...
        val firstMatch = goodPatterns
          .flatMap(pattern => Try(DateTimeFormat.forPattern(pattern).parseDateTime(value.string)).toOption)
          .headOption
        firstMatch match {
    ...
    }).toEither.left.map(exc => GovNotifyError(exc.getMessage))
  ...
  @tailrec
  final def applyFunctions(value: TemplateLookup, functionList: List[String]): Either[GovNotifyError, TemplateLookup] =
    functionList match {
