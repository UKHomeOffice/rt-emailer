...
class GovNotifyEmailSender(implicit appContext: AppContext) extends StrictLogging {
  ...
  def useGovNotify(email: Email): IO[Either[GovNotifyError, Boolean]] =
    getTemplate(email).map {
      case Right(Some(template)) =>
        val usesEmailPersonalisation =
          getPersonalisationsRequired(template).exists(_.startsWith("email:personalisations"))
        ...
        (usesEmailPersonalisation, hasEmailPersonalisation) match {
          case (true, false) =>
            Right(
              false
            ) /* if the email uses email:personalisation fields but our email has none, use legacy sending method. (only happens with resend behaviour) */
  ...
  def buildConfigPersonalisations(
      personalisationsRequired: List[String],
      templateName: String
  ): Either[GovNotifyError, Map[String, String]] = {
    def resolveConfigValue(configName: String): Either[GovNotifyError, String] =
      appContext.config.hasPath(s"govNotify.staticPersonalisations.$configName") match {
        case true =>
          Try(appContext.config.getString(s"govNotify.staticPersonalisations.$configName")).toEither.left.map(exc =>
            GovNotifyError(s"Config error govNotify.staticPersonalisations.$configName: ${exc.getMessage}")
          )
        case false =>
  ...
  def buildObjectPersonalisations(
      personalisationsRequired: List[String],
      obj: MongoDBObject,
      templateName: String,
      prefix: String
    ...
    val allResolvedPersonalisations =
      personalisationsRequired.filter(_.startsWith(s"$prefix:")).map { personalisationRequired =>
        ...
        val fieldValue: TemplateLookup = extractDBField(obj, fieldName) match {
          ...
          case None =>
            logger.warn(
              s"gov notify personalisation warning: missing field: $personalisationRequired (templateName: $templateName)"
            )
            TString("")
        }
        ...
        new TemplateFunctions().applyFunctions(fieldValue, functionList) match {
          ...
          case Right(list) =>
            logger.warn(
              s"gov notify personalisation warning: $personalisationRequired. error: lookup doesn't result in string. missing date/csvList call? (templateName: $templateName)"
            )
            Right((personalisationRequired, list.stringValue()))
          case Left(govNotifyError) =>
            logger.warn(
              s"gov notify personalisation warning: $personalisationRequired. error: $govNotifyError (templateName: $templateName)"
            )
  ...
  def buildCasePersonalisations(
      personalisationsRequired: List[String],
      email: Email,
      templateName: String
  ): IO[Either[GovNotifyError, (Map[String, String], Option[MongoDBObject])]] =
    appContext.database.caseObjectFromEmail(email).map {
      case Right(Some(caseObj)) =>
        buildObjectPersonalisations(personalisationsRequired, caseObj, templateName, "case").map { m =>
          (m, Some(caseObj))
        }
      case Right(None) =>
  ...
  def buildEmailPersonalisations(
      personalisationsRequired: List[String],
      email: Email,
      templateName: String
  ): Either[GovNotifyError, Map[String, String]] =
  ...
  def buildParentPersonalisations(
      personalisationsRequired: List[String],
      maybeCaseObj: Option[MongoDBObject],
      templateName: String
  ): IO[Either[GovNotifyError, Map[String, String]]] =
    ...
    personalisationsRequired.exists(_.startsWith("parent:")) match {
      case true =>
        maybeCaseObj match {
          case Some(caseObj) =>
            appContext.database
              .parentObjectFromCaseObject(caseObj)
              .map {
                case Right(Some(parentCaseObj)) =>
                  buildObjectPersonalisations(personalisationsRequired, parentCaseObj, templateName, "parent")
  ...
  def getPersonalisationsRequired(template: TemplateWC): List[String] = {
    val personalisationsRequired =
      template.getPersonalisation().asScalaOption().map(_.keySet.asScala.toList).getOrElse(List.empty)
    if appContext.config.getBoolean("app.templateDebug") then {
      logger.info(
        s"Personalisatons Required for ${template.getName()} (${template.getId()}): $personalisationsRequired"
      )
    }
  ...
  def getTemplate(email: Email): IO[Either[GovNotifyError, Option[TemplateWC]]] =
    notifyClientWrapper.getAllTemplates().map {
      _.map { allTemplates =>
    ...
    getTemplate(email).flatMap {
      case Right(Some(template)) =>
        ...
        val allPersonalisations = for
          (casePersonalisations, caseObj) <- EitherT(
            buildCasePersonalisations(personalisationsRequired, email, template.getName())
          )
          parentPersonalisations <- EitherT(
            buildParentPersonalisations(personalisationsRequired, caseObj, template.getName())
          )
          emailPersonalisations <- EitherT(
            IO.delay(buildEmailPersonalisations(personalisationsRequired, email, template.getName()))
          )
          configPersonalisations <- EitherT(
            IO.delay(buildConfigPersonalisations(personalisationsRequired, template.getName()))
          )
        yield casePersonalisations ++ parentPersonalisations ++ emailPersonalisations ++ configPersonalisations
                  ...
                  notifyClientWrapper
                    ...
                    .map {
                      case Right(response) =>
                        ...
                        logger.info(
                          s"Email sent via Gov Notify. Notification Id: ${response.getNotificationId()}, gov notify reference: ${govNotifyRef}, email table id: ${email.emailId}, template: ${response.getTemplateId()}, template version: ${response.getTemplateVersion()}"
                        )
                        Sent(
                          newText = Some(templatePreview.getBody()),
                          newHtml = templatePreview.getHtml().asScalaOption()
                        )
                      case Left(govNotifySendError) if govNotifySendError.transient && exhaustedRetries(email) =>
                        logger.error(
                          s"Cannot send email ${email.emailId}. Error during send: $govNotifySendError. Exhausted Retries"
                        )
                        ExhaustedRetries
                      ...
                      case Left(govNotifySendError) if !govNotifySendError.transient =>
                        logger.error(
                          s"Cannot send email ${email.emailId} via GovNotify due to partial success/failure. To avoid spamming, delivery will not be retried. Error during send: $govNotifySendError"
                        )
                        PartialError(govNotifySendError.message)
                    }
                case Left(govNotifyTemplateError) =>
                  logger.error(
                    s"Cannot send email ${email.emailId}. Error generating template preview: $govNotifyTemplateError"
                  )
                  IO.delay(Waiting)
              }
        }
  ...
  def exhaustedRetries(email: Email): Boolean =
    email.date.isBefore(
      appContext.nowF().minusHours(Duration(appContext.config.getString("govNotify.exhaustedTimeout")).toHours.toInt)
    )
}
