...
class PostgresDatabase(config: Config) extends Database with StrictLogging {
  ...
  def jsonToMongo(json: Json): MongoDBObject = {
    ...
    jsonFields.foreach { case (key, jsonValue) =>
      jsonValue match {
  ...
  def getWaitingEmails(): fs2.Stream[IO, Email] = {
    val selectSQL = sql"SELECT emailId, status, email FROM email WHERE status = $varchar"
      .query(varchar ~ varchar ~ jsonb)
      .map { case emailId ~ status ~ json => (emailId, status, json) }

    val ses: fs2.Stream[IO, skunk.Session[IO]] = fs2.Stream.resource(makeSession())
    val pc: fs2.Stream[IO, skunk.PreparedQuery[IO, String, (String, String, Json)]] =
      ses.flatMap(s => fs2.Stream.eval(s.prepare(selectSQL)))
  ...
  private def updateStatusWithContent(
      emailId: String,
      newStatus: String,
      newText: String,
      newHtml: String
  ...
  private def updateStatusNoContent(emailId: String, newStatus: String): IO[String] = {
    val updateCommand: Command[String] =
      newStatus match {
        case STATUS_WAITING =>
          sql"""UPDATE email SET status='WAITING', email = jsonb_set(email, '{status}', '"WAITING"') WHERE emailId=$varchar""".command
        case STATUS_SENT =>
          sql"""UPDATE email SET status='SENT', email = jsonb_set(email, '{status}', '"SENT"') WHERE emailId=$varchar""".command
        case _ =>
          sql"""UPDATE email SET status='ERROR', email = jsonb_set(email, '{status}', '"ERROR"') WHERE emailId=$varchar""".command
      }
  ...
  def caseObjectFromEmail(email: Email)(implicit
      appContext: AppContext
  ): IO[Either[GovNotifyError, Option[MongoDBObject]]] =
    IO.delay(Right(None))

  def parentObjectFromCaseObject(caseObj: MongoDBObject)(implicit
      appContext: AppContext
  ): IO[Either[GovNotifyError, Option[MongoDBObject]]] =
    IO.delay(Right(None))
}
