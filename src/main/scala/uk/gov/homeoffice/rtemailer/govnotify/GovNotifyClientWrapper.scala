...
class GovNotifyClientWrapper(implicit appContext: AppContext) extends StrictLogging {
  ...
  private def getAllClientTemplates(client: NotificationClient): IO[Either[GovNotifyError, List[TemplateWC]]] =
    IO.blocking(
      Try(client.getAllTemplates("email").getTemplates().asScala.toList).toEither
        .map { templates =>
        ...
        .left
        .map(exc => GovNotifyError(s"Error calling GovNotify.getAllTemplates: ${exc.getMessage}"))
    )

  def generateTemplatePreview(
      twc: TemplateWC,
      personalisations: Map[String, String]
  ): IO[Either[GovNotifyError, TemplatePreview]] =
    IO.blocking(
      Try(
        twc.client.generateTemplatePreview(
          twc.template.getId().toString(),
          personalisations.asJavaMap()
        )
      ).toEither match {
        case Left(exc) =>
          appContext.updateAppStatus(
            _.recordGovNotifyError(s"Error calling GovNotify.generateTemplatePreview: ${exc.getMessage}")
          )
  ...
  def sendEmail(
    ...
    emailsSent.map { list =>
      ...
      (errorList, goodResponseList) match {
        ...
        case (errorList, Nil)              =>
          errorList.zipWithIndex.foreach { case (err, idx) =>
            logger.error(s"Multiple errors and no success sending govNotify email: ${email.emailId}. $idx: $err")
          }
          Left(
            GovNotifyError(
              s"Multiple errors and no success sending govNotify email: ${email.emailId}. (System will retry)",
              transient = true
            )
          )
        case (errorList, successList) =>
          errorList.zipWithIndex.foreach { case (err, idx) =>
            logger.error(s"Mixed error/success sending govNotify email: ${email.emailId}. index: $idx: $err")
          }
          successList.zipWithIndex.foreach { case (good, idx) =>
            logger.info(
              s"Success for partial email part: ${email.emailId}. index: $idx: ${good.getReference().asScalaOption()}"
            )
          }
          Left(
            GovNotifyError(
              s"Mixed error/success sending govNotify email: ${email.emailId}. (Permanent failure)",
              transient = false
            )
          ) // transient flag critical importance to stop spamming emails to people!
      }
    }
  }
